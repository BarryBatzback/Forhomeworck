...
class UEFY_OT_setup_character_bones(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for k, v in mapping.items():
            ...
            if t_bone is None:
                self.report({'INFO'},
                            'Bone Not Found. Target might already have updated names or Wrong Character type was selected')
            else:
                t_bone.name = v
...
class UEFY_OT_setup_character_pose(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if source is None:
            self.report({'ERROR'},
                        'Invalid Source. UE_Mann. Armature in the dropdown list if not valid. Refresh and select again')
            return {'FINISHED'}
        if target is None:
            self.report({'ERROR'},
                        'Invalid Target. Character Armature in the dropdown list if not valid. Refresh and select again')
            return {'FINISHED'}
...
class UEFY_OT_setup_metarig_pose(bpy.types.Operator):
    ...
    def add_twist_bone(self, bone_name, owning_bone_name, layers):
        ...
        target = bpy.data.objects.get(self.target_name)
        print('Adding Twist bone: ' + bone_name + ' To Target: ' + target.name)
    ...
    def roll_metabones(self, source, target, s_bone_name, t_bone_name, axis_type):

        s_bones = source.data.edit_bones
        t_bones = target.data.edit_bones

        sbone = s_bones.get(s_bone_name)
        tbone = t_bones.get(t_bone_name)

        if sbone is None or tbone is None:
            return

        vec = self.get_axis_vec(sbone, axis_type)
        UEFY_FUNC_align_x_axis(tbone, vec)
    ...
    def execute(self, context):
        ...
        for k, v in bone_rolls.items():
            ...
            tbone_name = k
            roll = v
            self.roll_metabones(source, temp_rig, sbone_name, tbone_name, roll)
        ...
        bpy.ops.armature.select_all(action='SELECT')
        result = bpy.ops.armature.symmetrize(direction='POSITIVE_X')
        print(result)
        bpy.ops.armature.select_all(action='DESELECT')
        bpy.ops.armature.layers_show_all(all=True)
